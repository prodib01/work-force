...
export default function App() {
  ...
  const openEditModal = (company) => {
    ...
    setCompanySize(convertBackendCompanySize(company.company_size) || "");
    setHeadquarters(company.headquarters || "");
    setFoundedYear(company.year_founded?.toString() || "");
    setCompanyStructure(
      convertBackendToFrontendFormat(company.company_structure) || ""
    );
    setWorkEnv(convertBackendToFrontendFormat(company.work_environment) || "");
    setCommStyle(
      convertBackendToFrontendCommStyle(company.communication_style) || ""
    );
    setTeamStructure(company.team_structure_overview || "");
  ...
  const convertCompanySize = (size) => {
    ...
    return sizeMap[size] || "1-5";
  };

  // Convert backend options to frontend format
  const convertBackendCompanySize = (size) => {
    const sizeMap = {
      "1-5": "1-10",
      "6-10": "1-10",
      "11-50": "11-50",
  ...
  const convertBackendToFrontendFormat = (str) => {
    if (!str) return "";
  ...
  const convertBackendToFrontendCommStyle = (style) => {
    // Map backend options to frontend options
    const styleMap = {
  ...
  const saveContext = async () => {
    try {
      ...
      const url = isEditMode
        ? `${BASE_URL}/auth/companies/${selectedCompany.id}/`
        : `${BASE_URL}/auth/companies/`;
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to ${isEditMode ? "update" : "add"} company: ${
            response.status
          } ${responseBody}`
        );
      }
      ...
      console.error(
        `Error ${isEditMode ? "updating" : "adding"} company:`,
        err
      );
  ...
  const deleteCompany = async () => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/auth/companies/${companyToDelete.id}/`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    <div className="min-h-screen bg-gray-50">
            ...
            {isLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {companies.map((company) => (
                  ...
                  >
                    ...
                    <div className="text-sm text-gray-500 mb-1">
                      <span className="inline-block mr-2">
                        Location: {company.headquarters || "Not specified"}
                      </span>
                    </div>

                    <div className="text-sm text-gray-500 mb-1">
                      <span className="inline-block">
                        Founded: {company.year_founded || "Not specified"}
                      </span>
                    </div>
              ...
              <div className="flex flex-col items-center justify-center py-12">
                ...
                <p className="text-lg font-medium text-gray-500">
                  No company contexts found
                </p>
                <p className="text-sm mt-1 text-gray-400">
                  Click the "Add Company Context" button to create a new context
                </p>
              </div>
            )}
          </div>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full animate-fadeIn p-6">
      ...
      {/* Modal for Adding/Editing Company Context */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-fadeIn">
            <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50">
              <div className="flex justify-between items-center">
                <h2 className="text-xl font-bold text-gray-800 flex items-center">
                  ...
                  {isEditMode ? "Edit Company Context" : "Add Company Context"}
                </h2>
                <button
                  onClick={closeModal}
            ...
            <div className="p-6 border-t border-gray-200 flex justify-end gap-3 bg-gray-50">
              ...
              <button
                ...
                } text-white font-medium py-2 px-6 rounded-lg transition-colors shadow-sm flex items-center`}
